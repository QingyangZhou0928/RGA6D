    ...
    def forward(
        ...
        if attention_masks is not None:
            if attention_masks.dim() == 3:
                attention_masks = attention_masks.unsqueeze(1).expand(
                    -1, self.num_heads, -1, -1
                )  # (B, H, N, M)
                valid = attention_masks.sum(dim=-1, keepdim=True) > 0
            attention_scores = attention_scores.masked_fill(~attention_masks, float('-inf'))
            attention_scores = attention_scores.masked_fill(~valid, 0.0)
...
class AttentionLayer(nn.Module):
    def __init__(self, d_model, num_heads, dropout=None, learnable_residual_scale=False):
    ...
    def forward(
        ...
        if self.learnable_residual_scale:
            output_states = self.norm(self.residual_scale * hidden_states + input_states)
        else:
