...
def weighted_circle_loss(
    ...
    valid_row = row_masks.any()
    valid_col = col_masks.any()

    loss_row_mean = loss_row[row_masks].mean() if valid_row else 0.
    loss_col_mean = loss_col[col_masks].mean() if valid_col else 0.

    loss = (loss_row_mean + loss_col_mean) / 2
    # loss = (loss_row[row_masks].mean() + loss_col[col_masks].mean()) / 2
