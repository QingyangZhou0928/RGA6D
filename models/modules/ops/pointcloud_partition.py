...
from models.modules.ops.pairwise_distance import pairwise_distance
from models.modules.ops.index_select import index_select
...
def point_to_node_partition(
    points: torch.Tensor,  # points_f 
    nodes: torch.Tensor,  # points_c 
    point_limit: int,
    ...
    """
    sq_dist_mat = pairwise_distance(nodes, points)  # (_c, _f)

    point_to_node = sq_dist_mat.min(dim=0)[1]  # (_f,)
    node_masks = torch.zeros(nodes.shape[0], dtype=torch.bool).cuda()  # (_c,)
    node_masks.index_fill_(0, point_to_node, True)  # (_c,) _c

    matching_masks = torch.zeros_like(sq_dist_mat, dtype=torch.bool)  # (_c, _f)
    point_indices = torch.arange(points.shape[0]).cuda()  # [0,1,...,_f-1]
    matching_masks[point_to_node, point_indices] = True  # (_c, _f)
    sq_dist_mat.masked_fill_(~matching_masks, 1e12)  # (_c, _f)

    node_knn_indices = sq_dist_mat.topk(k=point_limit, dim=1, largest=False)[1]  # (_c, K) 
    node_knn_node_indices = index_select(point_to_node, node_knn_indices, dim=0)  # (_c, K) 
    node_indices = torch.arange(nodes.shape[0]).cuda().unsqueeze(1).expand(-1, point_limit)  # (M, K)
    node_knn_masks = torch.eq(node_knn_node_indices, node_indices)  # (_c, K) 
    node_knn_indices.masked_fill_(~node_knn_masks, points.shape[0])  # (_c, K) 
