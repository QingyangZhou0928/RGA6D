...
from models.modules.ops import grid_subsample, radius_search
from models.utils.torch import build_dataloader


# Stack mode utilities


def precompute_data_stack_mode(points, lengths, neighbor_limits, num_stages=3, voxel_size=0.025, radius=0.0625):
...
def registration_collate_fn_stack_mode(
    ...
    collated_dict['features'] = feats
    if precompute_data:
        input_dict = precompute_data_stack_mode(points, lengths, neighbor_limits, num_stages, voxel_size, search_radius)
        collated_dict.update(input_dict)
    else:
        collated_dict['points'] = points
        collated_dict['lengths'] = lengths
