...
from models.utils.pointcloud import (
    apply_transform,
...
def compute_rotation_mse_and_mae(gt_rotation: np.ndarray, est_rotation: np.ndarray):
    ...
    gt_euler_angles = Rotation.from_dcm(gt_rotation).as_euler('xyz', degrees=True)
    est_euler_angles = Rotation.from_dcm(est_rotation).as_euler('xyz', degrees=True)
