...
class RGA(nn.Module):
    ...
    def forward_coarse(self, data_dict, output_dict):
        ...
        output_dict['gt_node_corr_indices'] = gt_node_corr_indices  # loss val
        output_dict['gt_node_corr_overlaps'] = gt_node_corr_overlaps  # loss val
        ...
        with torch.no_grad():
            if not self.training:
                ...
                if all_region_ref_corr_indices is None and all_region_src_corr_indices is None and region_to_corr_indices is None and region_to_corr_mask is None and all_region_to_corr_scores is None and corr_scores is None:  
                    raise ValueError("CANNOT FIND REGION CONTAINING COHERENT correspondences")
                ...
                ref_node_corr_indices, src_node_corr_indices, node_corr_scores, ref_centroid_pts, src_centroid_pts = self.region_outlier_rejection(corr_ref_pts,
                                                                corr_src_pts,
                                                                region_to_corr_indices,  # (R_ref'',C)
                ...
                if ref_node_corr_indices.shape[0]!=0:
                    for i in range(ref_node_corr_indices.size(0)):
                        group_ref_node_corr_indices = ref_node_corr_indices[i,:]  # (C)ref-idx
                        group_ref_node_corr_indices = group_ref_node_corr_indices[group_ref_node_corr_indices!=-1]
                        group_src_node_corr_indices = src_node_corr_indices[i,:]  # (C)src-idx
                        group_src_node_corr_indices = group_src_node_corr_indices[group_src_node_corr_indices!=-1]
                        group_node_corr_scores = node_corr_scores[i,:]  # (C)scores
                    ...
                    output_dict['ref_centroid_pts'] = ref_c_center_pos.unsqueeze(0)  # (1,R_ref'',3)
                    output_dict['src_centroid_pts'] = src_c_center_pos.unsqueeze(0)  # (1,R_src'',3)
            else:
                ...
                for i in range(ref_centroid_pts.size(0)):
                    aug_transform = random_sample_transform(self.rotation_magnitude, self.translation_magnitude)    # transform:src1->ref
                    aug_transform = torch.tensor(aug_transform).cuda().float()
                    inv_aug_transform = inverse_transform(aug_transform)  # inv_transform:ref->src1
                    inv_aug_transform = inv_aug_transform.cuda().float()
                    estimated_transform[i,:,:] = inv_aug_transform @ transform  # src->src1
                output_dict['coarse_est_transform'] = estimated_transform
    ...
    def forward_fine(self, data_dict, output_dict, group_id):
        ...
        group_init_T = init_transform[group_id]  # src->src1
        ...
        with torch.no_grad():
            group_attention = group_attention_list[-1].float()  
            group_assginment_mat = torch.softmax(group_attention, dim=2) * torch.softmax(group_attention, dim=1)
            group_ref_score = group_score_list[-1][:,:group_ref_f_pts.shape[0]].float()  
            group_src_score = group_score_list[-1][:,group_ref_f_pts.shape[0]:].float()
            group_ref_score = group_ref_score[:, :, None].repeat(1, 1, group_src_f_pts.shape[0])  
            group_src_score = group_src_score[:, None, :].repeat(1, group_ref_f_pts.shape[0], 1)  
            ...
            group_estimated_transform = self.procrustes(group_pred_src_f_pts.squeeze(0), group_ref_f_pts, group_assginment_score.squeeze(0))  # src1->ref
