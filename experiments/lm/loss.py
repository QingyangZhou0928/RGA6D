...
class RegionCoarseMatchingLoss(nn.Module):
    ...
    def forward(self, output_dict):
        ...
        pos_masks = torch.gt(gt_region_overlaps, self.positive_overlap)  
        neg_masks = torch.eq(gt_region_overlaps, 0)  
        pos_scales = torch.sqrt(gt_region_overlaps * pos_masks.float()) 
...
class FineLoss(nn.Module):
    ...
    def forward(self, output_dict, data_dict): 
        ...
        ref_node_corr_knn_points = output_dict['ref_corr_points'][-1].unsqueeze(0) 
        src_node_corr_knn_points = output_dict['src_corr_points'][-1].unsqueeze(0)
...
class Evaluator(nn.Module):
    ...
    def evaluate_fine(self, output_dict, data_dict, top_idx,positive_radius=0.005):
        ...
        fg_label1 = (dis1 <= positive_radius ** 2).float()  # (min dist) & (< thr)
    ...
    def transform_ranking(self,data_dict,output_dict):
        ...
        dist = torch.cdist(ref_points[None,:,:].expand(N_T,-1,-1),batch_src_points)  # pairwise distance (N_T, N_c, N_pts)
